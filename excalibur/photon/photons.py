...
class Photons:
    ...
    *,
    ...
    def _orthonormal_screen_basis_from_k(k_cart: np.ndarray):
        """Return an orthonormal basis (e1, e2, k) with e1,e2 perp k.
    ...
    def _solve_u0_from_null_condition(self, origin: np.ndarray, u_spatial: np.ndarray, *, sign: float = -1.0) -> float:
        """Solve for u0 using g_{mu_nu} u^mu u^nu = 0 assuming metric is (approximately) diagonal.
    ...
    def generate_impact_parameter_bins(
        ...
        *,
        ...
        """Generate photons by sampling impact parameter bins on a screen perp central_direction.
