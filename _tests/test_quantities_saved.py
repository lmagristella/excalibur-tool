...
def check_quantities_in_trajectory(filename):
    ...
    with h5py.File(filename, 'r') as f:
        ...
        if photon_0_states.shape[1] < 14:
            print("\n[FAIL] NO QUANTITIES SAVED (only 8 columns: position + velocity)")
            return False
        
        print("\n[ok] Trajectory has 14+ columns (quantities present)")
        ...
        print(f"  Coordinates [0-3]: eta={first_valid_row[0]:.3e}, x={first_valid_row[1]:.3e}, y={first_valid_row[2]:.3e}, z={first_valid_row[3]:.3e}")
        ...
        print(f"    [9]     phi/c^2 (potential): {phi:.6e}")
        print(f"    [10-12] grad_phi (m/s^2):    [{grad_phi_x:.3e}, {grad_phi_y:.3e}, {grad_phi_z:.3e}]")
        print(f"    [13]    phi_dot/c^2:         {phi_dot:.6e}")
        
        # Validate
        if np.isnan(a) or np.isnan(phi):
            print("\n[FAIL] QUANTITIES ARE NaN (not properly computed)")
            return False
        
        if a <= 0 or a > 2:
            print(f"\nWARNING:  WARNING: Scale factor a={a:.6f} seems wrong (should be ~0.2-1.0)")
        
        print("\n[ok] Quantities are valid!")
        ...
        if n_nan_a > 0 or n_nan_phi > 0:
            print("\nWARNING:  Some quantities are NaN (partial failure)")
            return False
        
        print("\n[ok] ALL QUANTITIES VALID ACROSS ENTIRE TRAJECTORY")
...
if __name__ == "__main__":
    ...
    if success:
        print("\n" + "="*70)
        print("TEST PASSED: Quantities properly saved [ok]")
        print("="*70)
    else:
        print("\n" + "="*70)
        print("TEST FAILED: Quantities not saved or invalid [FAIL]")
        print("="*70)
        sys.exit(1)
