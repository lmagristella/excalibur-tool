#!/usr/bin/env python3
"""
Test parallel speedup with different numbers of workers.
Measures actual performance gain from multiprocessing.
"""
...
from excalibur.integration.parallel_integrator import ParallelIntegratorChunked
...
def run_tests():
    ...
    # Generate test photons (more photons for better parallel testing)
    n_test_photons = 20
    print(f"\nGenerating {n_test_photons} test photons...")
    observer_4d = np.array([observer_eta, *observer_position])

    photons_sets = []
    for n_workers in [1, 2, 4, cpu_count()-1 if cpu_count() > 1 else 1]:
        photons = Photons()
    ...
    for n_workers, photons in photons_sets:
        print(f"--- Testing with {n_workers} worker(s) ---")
        ...
        if n_workers == 1:
            ...
            parallel_integrator = ParallelIntegratorChunked(
                ...
                dt=dt,
                n_workers=n_workers,
                chunk_size=max(1, n_test_photons // n_workers)
            )
            
            start = time.time()
            success = parallel_integrator.integrate_photons_chunked(
    ...
    print("\nAnalysis:")
    print(f"  Best speedup achieved: {max(r[1]/results[0][1] for r in results):.2f}x")
    print(f"  Theoretical maximum: {cpu_count()-1}x (with {cpu_count()-1} workers)")
    print(f"  Overhead from multiprocessing: ~{(1 - max(r[1]/results[0][1] for r in results[1:])/max(r[0] for r in results))*100:.1f}%")
