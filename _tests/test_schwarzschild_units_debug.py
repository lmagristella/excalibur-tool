#!/usr/bin/env python3
"""
Test de diagnostic pour les unites dans la simulation Schwarzschild.
Verifie l'initialisation des photons et les premieres etapes d'integration.
"""
...
def test_schwarzschild_units():
    ...
    # Parametres physiques en unites coherentes
    mass_msun = 1e15
    mass_kg = mass_msun * one_Msun
    
    # Positions en Mpc, converties en metres pour la metrique
    ...
    print(f"1. Parametres physiques:")
    print(f"   Masse: {mass_msun:.1e} Msun = {mass_kg:.2e} kg")
    ...
    # Test de la metrique a la position de l'observateur
    ...
    # Generation d'un photon de test
    ...
    if len(photons_manager.photons) > 0:
        ...
        # Test quelques etapes d'integration
        integrator = Integrator(metric, dt=-1e13)  # Pas de temps negatif pour remonter le temps
        
        print(f"\n7. Test integration (10 etapes):")
        print(f"   Position initiale: {photon.x[1:]/one_Mpc}")
        
        # Sauvegarde etat initial
        initial_pos = photon.x.copy()
        
        # Integration de quelques etapes
        try:
            integrator.integrate(photon, 10000)
            
            print(f"   Position finale: {photon.x[1:]/one_Mpc}")
            print(f"   Nombre d'etats enregistres: {len(photon.history.states)}")
            ...
            if len(photon.history.states) > 1:
                ...
                # Verifier si la distance est raisonnable
                if distance_total/one_Mpc > 1000:  # Plus de 1000 Mpc
                    print(f"   WARNING:  Distance ABERRANTE detectee!")
                    return False
                else:
                    print(f"   [ok] Distance raisonnable")
                    return True
            else:
                print(f"   [FAIL] Pas d'historique enregistre")
                return False
...
if __name__ == "__main__":
    ...
    if not success:
        print("\n[FAIL] TEST ECHOUE - Probleme detecte dans l'integration Schwarzschild")
        sys.exit(1)
    else:
        print("\n[ok] TEST REUSSI - Integration Schwarzschild normale")