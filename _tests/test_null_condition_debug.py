...
print(f"   eta = {eta_test:.2e} s = {eta_test/one_Gyr:.1f} Gyr")
print(f"   a(eta) = {a_test:.6f}")
print(f"   a^2 = {a_test**2:.3e}")
...
print(f"\n2. Grid check")
print(f"   Grid: {N}^3")
print(f"   Phi range: [{phi_field.min():.2e}, {phi_field.max():.2e}] m^2/s^2")
...
print(f"   Position: eta={observer_eta:.2e}, (0, 0, 0)")
print(f"   a(eta) = {a_of_eta(observer_eta):.6f}")
print(f"   Phi(observer) = {phi_obs:.3e} m^2/s^2")
print(f"   Psi = Phi = {psi_obs:.3e}")
print(f"   1 + 2Psi = {1 + 2*psi_obs:.3e}")
print(f"\n   Metric tensor:")
...
print(f"\n   PROBLEM IDENTIFIED:")
print(f"   g[0,0] expected = -a^2(1+2psi)c^2 = {g00_expected:.6e}")
print(f"   g[0,0] current  = -a^2(1+2psi)/c^2 = {g00_current:.6e}")
print(f"   Ratio = {g00_current / g00_expected:.2e}")
print(f"   ERROR: Factor c^4 = {c**4:.2e} wrong!")
...
# For null geodesic: -c^2(u^0)^2 + a^2(u_spatial)^2 = 0
# So: u^0 = +/-(a/c) * |u_spatial|
...
# Calculate u^0 from null condition: g_mu_nu u^mu u^nu = 0
# With the current (buggy) metric: g[0,0] = -a^2(1+2psi)/c^2
# Null condition: g[0,0](u^0)^2 + g[i,i](u^i)^2 = 0
# So: (u^0)^2 = -g[i,i](u^i)^2 / g[0,0]
...
print(f"   u^0 from null condition = {u0_from_null:.3e}")
print(f"   u^0 from formula        = {u0:.3e}")
...
print(f"   u^0 = {u0:.3e}")
print(f"   u^1 = {u_spatial[0]:.3e}")
print(f"   u^2 = {u_spatial[1]:.3e}")
print(f"   u^3 = {u_spatial[2]:.3e}")
...
print(f"   u*u = g_mu_nu u^mu u^nu = {null_cond:.3e}")
print(f"   Expected: ~ 0")
print(f"   Status: {'[ok] OK' if abs(null_cond) < 1e-10 else '[FAIL] WRONG'}")

print(f"Is the gravitational field over c**2 << 1 ?")
print(f"   Phi/c^2 = {phi_obs:.3e}")
...
print(f"\n   Breakdown:")
print(f"   g00(u^0)^2 = {g00_u0_sq:.3e}")
print(f"   g_ii(u^i)^2 = {gii_uspatial:.3e}")
...
print(f"   g00(u^0)^2 = {g00_correct_contribution:.3e}")
print(f"   Sum      = {correct_norm:.3e}")
print(f"   Status: {'[ok] NULL' if abs(correct_norm) < 1e-10 else '[FAIL] NOT NULL'}")
...
print("\nFactor c^4 error causes null condition to fail!")
print("="*70)
