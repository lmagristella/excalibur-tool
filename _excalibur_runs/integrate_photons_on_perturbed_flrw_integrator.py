...
def integrate_single_photon_old(args):
    """Helper function to integrate a single photon for multiprocessing."""
    photon_idx, photon_data, integrator_params = args
    
    # Create a local integrator for this process
    local_integrator = Integrator(
        metric=integrator_params['metric'],
        dt=integrator_params['dt'],
        mode="sequential",  # Force sequential for individual photons
    ...
    # Create photon from data
    photon = Photon(
        position=photon_data['position'],
        direction=photon_data['u']  # u is the direction
    )
    photon.record()  # Record initial state
    
    # Integrate single photon
...
def main_test():
    ...
    print("=== Backward Ray Tracing with Grid-managed Shared Memory ===")

    # ----------------------
    # 0. Integration parameters
    # ----------------------
    integrator_type = "rk4"
    integration_mode = "sequential"
    n_workers = min(cpu_count(), 5)

    stop_mode = "steps"
    stop_value = 500  # Will be recalculated
    ...
    _ = cosmology.a_of_eta(1e18)  # initialize
    ...
    # Add to Grid (shared memory)
    grid.add_field("Phi", phi_field, shared=True)

    # ----------------------
    # 3. Metric
    # ----------------------
    # Will be re-created locally in each worker
    interpolator = InterpolatorFast(grid)
    metric = PerturbedFLRWMetricFast(a_of_eta=a_of_eta, grid=grid, interpolator=interpolator)

    # ----------------------
    # 4. Photon generation
    # ----------------------
    observer_eta = eta_present
    observer_position = np.array([dx, dy, dz])
    direction_to_mass = center - observer_position
    direction_to_mass /= np.linalg.norm(direction_to_mass)
    ...
    n_steps = int(grid_size / (c * abs(dt_initial)))
    stop_value = n_steps
    ...
    for i, photon in enumerate(photons):
        photon_data = {'position': photon.x.copy(), 'u': photon.u.copy()}
        photon_args.append((i, photon_data, grid, a_of_eta, integrator_params))

    n_processes = min(n_workers, len(photons))
    with Pool(processes=n_processes) as pool:
        results = pool.map(integrate_single_photon, photon_args)

    successful_integrations = 0
    for photon_idx, integrated_photon, success in results:
        if success > 0:
            photons.photons[photon_idx].history = integrated_photon.history
    ...
    print(f"[DONE] Backward ray tracing: {successful_integrations}/{len(photons)} photons")
    print(f"Total time: {total_time:.2f}s, output saved to {output_filename}")
...
def main():
    ...
    integration_mode = "sequential"  # TEST: Use script-level multiprocessing
    ...
    if stop_mode == "steps":
        stop_value = 500  # Restored: 500 steps for production
    ...
    grid_size = 1000 * one_Mpc / h # 1000 Mpc/h box size
    dx = dy = dz = grid_size / N
    ...
    metric = PerturbedFLRWMetricFast(cosmology.a_of_eta, grid, interpolator, analytical_geodesics=False)  # OPTIMIZED: Pass cosmology for fast adot
    ...
    observer_eta = eta_present
    observer_position = np.array([dx,dy,dz])   # Observer location
    ...
    n_photons = 100  # Restored: 21 photons for performance test
    ...
    #computing n_steps so that photons reach approximately the opposite side of the grid
    n_steps = int((grid_size) / (c * abs(dt_initial)))
    stop_value = n_steps  # Update stop_value for 'steps' mode
    ...
    shm_name = grid._shared_fields['Phi'].name
    shape = grid.fields['Phi'].shape
    dtype = grid.fields['Phi'].dtype

    photon_args = []
    for i, photon in enumerate(photons):
        photon_data = {'position': photon.x.copy(), 'u': photon.u.copy()}
        photon_args.append((i, photon_data, shm_name, shape, dtype, integrator_params))
    
    # Choose integration approach based on mode
    integration_start = time.time()
    successful_integrations = 0
    
    if integration_mode == "sequential":
        # Use the new integrator's sequential mode directly (no multiprocessing)
        print(f"   Using sequential mode (single process)...")
        
        # Create integrator with sequential mode
        integrator = Integrator(
            metric=metric,
            dt=-dt_initial,  # Negative for backward
            mode="sequential",
            integrator=integrator_type,
            rtol=rtol,
            atol=atol,
        ...
        # Integrate all photons at once
        results = integrator.integrate(
            photons,  # Pass the photons object directly
            stop_mode=stop_mode, 
            stop_value=stop_value,
            verbose=True
        )
        
        successful_integrations = results
        
    else:
        # Use script-level multiprocessing (parallel mode)
        n_processes = min(n_workers, cpu_count(), len(photons))
        n_processes = cpu_count()
        n_processes = 2
        print(f"   Using {n_processes} processes for parallel integration...")
        
        with Pool(processes=n_processes) as pool:
            results = pool.map(integrate_single_photon, photon_args)
        
        # Update original photons with results
        for photon_idx, integrated_photon, success in results:
            if success > 0:
                # Copy the integrated photon's history back to original
                photons.photons[photon_idx].history = integrated_photon.history
                successful_integrations += 1
