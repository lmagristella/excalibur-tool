...
Memory requirement: ~9 GB for the potential field (float64, 1024³).
...
def main():
    ...
    # Use chunks to avoid allocating three N³ arrays at once (saves ~25 GB)
    ...
    outfile = os.path.join(outdir, "lensing_nfw_cosmo_results.npz")
    ...
    if mask.any():
        ...
        if len(ratio) > 0:
            print(f"  Δκ_num / κ_NFW (b < R_200): {ratio.mean():.3f} ± {ratio.std():.3f}")
