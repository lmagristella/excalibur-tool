...
Reads ``lensing_nfw_results.npz`` and produces:
    1.  κ(b) radial profile  vs  NFW analytic  (linear + log-log)
    2.  |γ|(b) radial profile vs  NFW analytic  (linear + log-log)
    3.  2D convergence map   Δκ(b₁, b₂)
    4.  2D shear map         |γ|(b₁, b₂)
    5.  2D magnification map μ-1
...
# ------------------------------------------------------------------
#  Radial profiles  (4-panel : κ raw, Δκ lin, Δκ log, |γ| log)
# ------------------------------------------------------------------
def plot_radial_profiles(d, outdir):
    ...
    fig.suptitle(
        rf"NFW Lensing — $M_{{200}} = {float(d['M_200_Msun']):.0e}\,M_\odot$, "
        rf"$c = {c_NFW:.0f}$, "
        rf"$R_{{200}} = {R200:.2f}$ Mpc, "
        rf"$r_s = {rs*1000:.0f}$ kpc, "
        rf"Grid ${int(d['N_grid'])}^3$",
        fontsize=13, y=1.01,
    )
    fig.tight_layout()
    fname = os.path.join(outdir, "lensing_nfw_profiles.png")
...
# ------------------------------------------------------------------
#  Ratio plot  Δκ_num / κ_analytic
# ------------------------------------------------------------------
def plot_ratio(d, outdir):
    b  = d["b_profile_Mpc"]
    k  = d["kappa_profile"]
    ka = d["kappa_analytic"]
    ga = d["gamma_analytic"]
    ...
    # κ ratio
    ax = axes[0]
    ok = (b > 0.03) & (ka > 0) & np.isfinite(dk / ka)
    ratio_k = dk[ok] / ka[ok]
    ...
    ax.axvline(rs, color="orange", ls=":", lw=1, label=rf"$r_s$")
    ax.set_xlabel("$b$  [Mpc]")
    ax.set_ylabel(r"$\Delta\kappa_\mathrm{num}\;/\;\kappa_\mathrm{NFW}$")
    ax.set_title(r"Convergence ratio")
    ax.set_ylim(0.5, 1.5)
    ...
    fig.tight_layout()
    fname = os.path.join(outdir, "lensing_nfw_ratio.png")
    fig.savefig(fname, dpi=150, bbox_inches="tight")
    print(f"   ✓ {fname}")
    plt.close(fig)
...
def plot_maps(d, outdir):
    ...
    fig.suptitle(
        rf"NFW 2D Lensing Maps — $M_{{200}} = {float(d['M_200_Msun']):.0e}\,M_\odot$, "
        rf"$c = {float(d['c_NFW']):.0f}$",
        fontsize=13, y=1.02,
    )
    fig.tight_layout()
    fname = os.path.join(outdir, "lensing_nfw_maps.png")
    fig.savefig(fname, dpi=150, bbox_inches="tight")
    print(f"   ✓ {fname}")
    plt.close(fig)
...
def plot_distance_comparison(d, outdir):
    ...
    # -- Recover λ_S  (affine distance to source) -----------------------
    ...
    D_flat_prof = d["D_flat_profile"]   # shape (n_profile, 4), normalised by λ_S
    b_prof = d["b_profile_Mpc"]

    DA_ray_Mpc = np.empty(len(b_prof))
    for i in range(len(b_prof)):
        D_raw = D_flat_prof[i] * chi_S          # un-normalise → metres
    ...
    fig.suptitle(
        rf"Angular-diameter distance — NFW $M_{{200}} = {float(d['M_200_Msun']):.0e}\,M_\odot$, "
        rf"$c = {float(d['c_NFW']):.0f}$"
        rf" — $D_A^\mathrm{{FLRW}} = {DA_FLRW_Mpc:.1f}$ Mpc  ($z_s = {z_source:.3f}$)",
        fontsize=12, y=1.02,
    )
    fig.tight_layout()
    fname = os.path.join(outdir, "lensing_nfw_distance_comparison.png")
    fig.savefig(fname, dpi=150, bbox_inches="tight")
    print(f"   ✓ {fname}")
    plt.close(fig)
    ...
    if inner.any():
        print(f"     δD_A/D_A (b<R200): [{delta_DA[inner].min()*100:.4f}, "
...
def print_statistics(d):
    ...
    print("=" * 65)
    print(f"  Halo  : NFW, M_200 = {float(d['M_200_Msun']):.0e} M☉, "
    ...
    if 'Sigma_cr' in d:
        ...
        print(f"  Σ_cr = {Scr:.3e} M☉/Mpc²")
    print()
    print(f"  κ_bg (far field)    : {k_bg:.6e}")
    print(f"  Δκ max  (numerical) : {dk.max():.4e}")
    print(f"  κ  max  (analytic)  : {ka.max():.4e}")
    print(f"  |γ| max (numerical) : {g.max():.4e}")
    print(f"  γ_t max (analytic)  : {ga.max():.4e}")

    # Ratio inside R_200
    mask = (b > 0.03) & (b < R200) & (ka > 0)
    if mask.any():
        ratio_k = dk[mask] / ka[mask]
        ratio_k = ratio_k[np.isfinite(ratio_k)]
        if len(ratio_k) > 0:
            print(f"\n  Δκ_num / κ_NFW  (0.03 < b < R_200):")
